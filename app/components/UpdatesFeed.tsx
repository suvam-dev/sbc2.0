...
export default function UpdatesFeed({ updates = UPDATES_DATA }: UpdatesFeedProps) {
  ...
  return (
    <section className="bg-[#d2e5e9] py-6 sm:py-8 lg:py-9 border-b border-[#321F1F]/10">
      <div className="max-w-[1167px] mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex-1">
                  ...
                  {item.cta && (
                    <a
                      href={item.cta.href}
                      onClick={(e) => {
                        if (item.id === "mentor-registration-open" || item.cta?.href === "#partners") {
                          const el = document.getElementById("partners");
                          if (el) {
                            e.preventDefault();
                            el.scrollIntoView({ behavior: "smooth" });
                            window.dispatchEvent(new CustomEvent("open-mentor-modal"));
                          }
                        }
                      }}
                      className="font-semibold text-[#972933] hover:text-[#74001c] inline-flex items-center gap-1 group whitespace-nowrap transition-colors cursor-pointer"
                    >
        ...
        {/* Upcoming Group (only rendered if upcoming items exist) */}
